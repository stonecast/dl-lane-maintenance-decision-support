...
기존 인력 중심 도로 점검 방식의 한계를 보완하기 위해 딥러닝 기반 영상처리를 활용하여 차선 상태를 분석하고, <br>
유지보수 우선순위를 제안하는 시스템을 구현한 학부 프로젝트입니다.

#### 담당 역할
- U-Net 기반 딥러닝 모델 구현 및 학습
- 데이터 전처리 및 라벨링 기준 수립
- 모델 성능 평가(IoU)
- Django 기반 웹 시스템 구현
- TMAP API 연동 및 유지보수 우선순위 산정 로직 구현

#### 프로젝트 정보
- 프로젝트 기간: 2025.08 ~ 2025.11
- 프로젝트 인원: 2명
- 사용 기술: Python | PyTorch | OpenCV | Django | TMAP API
...
차선 영역과 결함 영역을 각각의 모델로 검출한 뒤 <br>
결함 비율을 계산하여 최종 정상/불량을 판단하도록 설계하였습니다.

또한 실제 서비스 형태로 구현하는 것을 목표로 하였기 때문에, <br>
추론 속도와 구현 난이도를 고려하여 비교적 가볍고 구현이 용이한 U-Net을 기반 모델로 선정하였습니다.
...
1. Model 1 : Lane Segmentation
    ...
    약 16,000장의 차선 Polygon 데이터로 학습하였으며, <br>
...
2. Model 2 : Defect Segmentation
    ...
    결함 데이터는 직접 라벨링이 필요한 데이터였으며,<br>
    학습 데이터의 수가 제한적이고, 결함의 형태가 다양하여 Lane Segmentation에 비해 상대적으로 낮은 성능을 보였습니다.
...
두 모델의 결과를 결합하여 차선별 결함 비율을 계산하고,<br>
...
최종적으로 이미지 단위가 아닌 차선 단위의 정상/불량 결과를 제공하도록 구현하였습니다.

일반화 성능 확인을 위해 약 1,000장의 별도 이미지에 대해 차선을 정상/불량으로 재분류한 후,<br>
...
초기에는 하나의 모델에서 정상/불량을 직접 분류하는 방식을 고려했습니다. <br>
그러나 차선 전체를 대상으로 분류를 수행할 경우 결함의 위치와 비율을 충분히 반영하지 못해 안정적인 결과를 얻기 어려웠습니다.

이를 해결하기 위해 차선 영역과 결함 영역을 각각 Segmentation한 뒤, <br>
차선 내부의 결함 비율을 계산하는 후처리 로직을 추가하였습니다. <br>
이를 통해 모델이 해결해야 하는 문제를 단순화하고, 보다 안정적으로 정상/불량을 판단하도록 설계하였습니다.

### 프로젝트를 통해 배운 점

프로젝트를 진행하며 딥러닝 모델 자체보다 문제를 분석하고 적절한 처리 과정을 설계하는 것이<br>
실제 시스템에서는 더욱 중요할 수 있다는 점을 배웠습니다.

이를 통해 모델과 영상처리 기반 후처리를 적절히 결합하여 <br>
문제를 해결하는 사고방식을 익힐 수 있었습니다.
